...
class BLGinSTM:
    ...
    def plot_dIdV(self,show=True,save=False,norm=False,inelastic=False):
        """
        ...
        if inelastic==True:
            ...
            dx = np.abs(self.VT[1]-self.VT[0])

            for i in range(np.shape(dIdV)[1]):
                for j in range(np.shape(dIdV)[0]):
                    x = np.linspace(0,self.VT[j],num=np.abs(j0-j))
                    a, b = min(j,j0), max(j,j0)
                    didv = dIdV[a:b,i]
                    if j>j0: didv = np.flip(didv)
                    if j==j0:
                        DdIdV[j,i]=0
                        continue
                    integrand = didv*gaussian(x)
    ...
    def plot_dIdV_waterfall(self,VT,VB):
        ...
        Currently does not include phonon gap
        """
